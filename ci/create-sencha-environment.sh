...
# `ci/create-sencha-package.sh` can actually build and add the BasiGX package.
#
...
# Create a folder for the BasiGX package
if [ ! -d $BASIGX_IN_SENCHA_WS_FOLDER ]; then
    mkdir -p $BASIGX_IN_SENCHA_WS_FOLDER
fi

# Initialize local repository
$SENCHA_CMD package repo init -name "$BASIGX_REPO_NAME" -email "$BASIGX_REPO_EMAIL"

# Add info about where to find GeoExt
