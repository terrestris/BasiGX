...
$SENCHA_CMD package add $SENCHA_WS/build/$BASIGX_PACKAGE_NAME/$BASIGX_PACKAGE_NAME.pkg

# TODO we could actually try to use the package now in an application that
#      wants to use the BasiGX package
...
mkdir -p $STATS_DIR
cp $SENCHA_WS/build/$BASIGX_PACKAGE_NAME/$BASIGX_PACKAGE_NAME.pkg $STATS_DIR
cd $STATS_DIR
unzip -q $BASIGX_PACKAGE_NAME.pkg

FILESIZE_PKG=$(du -k "$STATS_DIR/$BASIGX_PACKAGE_NAME.pkg" | cut -f1)
FILESIZE_JS_DEBUG=$(du -k "$STATS_DIR/build/$BASIGX_PACKAGE_NAME-debug.js" | cut -f1)
FILESIZE_JS=$(du -k "$STATS_DIR/build/$BASIGX_PACKAGE_NAME.js" | cut -f1)

echo "Size of $BASIGX_PACKAGE_NAME.pkg = $FILESIZE_PKG kb"
echo "Size of $BASIGX_PACKAGE_NAME.js = $FILESIZE_JS kb"
echo "Size of $BASIGX_PACKAGE_NAME-debug.js = $FILESIZE_JS_DEBUG kb"
